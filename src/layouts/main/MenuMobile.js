...
import { Box, List, Drawer, Collapse, Stack, ListItemText, ListItemIcon, Button, ListItemButton, IconButton } from "@mui/material";
// components
...
export default function MenuMobile({ isHome, navConfig }) {
  ...
  return (
    <>
      ...
      <Drawer anchor="top" open={drawerOpen} onClose={handleDrawerClose} ModalProps={{ keepMounted: true }} PaperProps={{ sx: { pb: 5, height: "100vh", backgroundImage: "none" } }}>
        <Scrollbar>
          ...
          <List disablePadding sx={{ mt: 3 }}>
            {navConfig.map((link) => (
              <MenuMobileItem key={link.title} item={link} isOpen={open} onClose={handleDrawerClose} onOpen={handleOpen} />
            ))}
          </List>
          <Stack sx={{ px: 2, mt: 5 }}>
